...
function listarTudo( int $id=null ): array//colocar int $id=null diz que esse parametro é opcional
{
	...
	return is_array($reg) ? $reg : [];//verifica se $reg está como array. se não ele transforma em um e retorna
...
function ja_existe_email(string $email): bool//cria function com paramentro de uma string $email que retorna valor booleano
{
	...
	return true; //verifica se id é numérico, ou seja, se tem id, se tiver é true senão é false. verificando em algo que eu sei que é numerico para ver se existe apenas.
